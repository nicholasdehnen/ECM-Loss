...
class EffectiveClassMarginLossNickWAuthorBgHandling(nn.Module):

    def __init__(self,
                 stats_file = None,
                 loss_weight = 1.0,
                 fg_bg_ratio = 6.3313, # taken from authors: empirical fg-bg ratio from Mask-RCNN.
                 **kwargs):
        """Effective class margin loss.
        Reimplemented by Nicholas Dehnen, according to the ECCV22 paper by Cho & Krähenbühl.
        Using the author's implementation regarding handling of the background class.
        ...
        """
        super(EffectiveClassMarginLossNickWAuthorBgHandling, self).__init__()
        ...
        self.num_classes = self.class_stats.shape[0]
        
        # complementary arrays of positive and negative examples for each class
        n_pos = np.concatenate((self.class_stats, self.class_stats.sum()[None]*fg_bg_ratio)) # from authors impl
        n_neg = np.sum(n_pos) - n_pos
        ...
        self.m_c = a_c * np.log((1 + a_c) / a_c) # simplification
        self.m_c = torch.tensor(self.m_c)
